...
function QuestionList({questions, setQuestions}) {
  const handleDeleteClick = async (id) => {
    ...
    const handleAnswerChange = async (selection, id) => {
      ...
      const res = await fetch(`https://localhost:4000${id}`).then(r => r.json())
    ...
    const questionMap = questions.map(question => <QuestionItem key={question.id} onAnswerChange={handleAnswerChange} onDeleteClick={handleDeleteClick} question={question}/>)
